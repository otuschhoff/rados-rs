# rados-rs: Native Rust RADOS Client Design and Execution Spec

Status: R00 repository/reference setup executed; see R00 status for validation and open gates. Rust implementation begins in R01.
...
Use the existing [P00 evidence manifest](../reference/go/docs/p00/evidence.json), not newly guessed versions:
...
Rust v1 targets every implemented, non-deferred public Go behavior and every native API mapping represented by that behavior. Use the [native API inventory](../reference/go/docs/p00/api-inventory.csv), [Go public API contract](../reference/go/docs/p01/public-api.md), [P12 API coverage](../reference/go/docs/p12/api-coverage.md), exported Go source and tests together. Historical inventory counts alone are not a parity metric.
...
| --- | --- | --- |
| [Protocol source index](../reference/go/docs/p00/protocol-sources.md), phase tasks and handoffs | Read exact Ceph/Go symbols and recorded decisions before each task | Add Rust symbols and evidence links; preserve source revisions |
...
| [Encoding](../reference/go/internal/encoding/codec.go), [wire types](../reference/go/internal/protocol/address.go), adjacent tests | Reference semantic contracts and edge cases | Explicit Rust byte parsing; do not expose wire structs as public API |
| [Messenger framing](../reference/go/internal/msgr/frame.go), [secure framing](../reference/go/internal/msgr/secure.go), [session core](../reference/go/internal/msgr/session.go) | Reuse transcripts, frame vectors and transition tests | Tokio I/O/task ownership, cancellation-safe writes and request registration |
| [Authentication](../reference/go/internal/cephx/core.go), [connector](../reference/go/internal/cephx/connector.go), [service connector](../reference/go/internal/cephx/service_connector.go) | Reuse challenge/ticket/authorizer vectors and live denial/renewal cases | Audited Rust crypto and credential lifetime design |
| [Map placement](../reference/go/internal/maps/placement.go), [CRUSH](../reference/go/internal/crush/place.go), [integer logarithm](../reference/go/internal/crush/ln.go) | Reuse whole object-to-primary corpus, not just CRUSH outputs | Exact wrapping arithmetic, widths, signed comparisons and integer math |
| [Mutation ownership/flush](../reference/go/internal/objecter/mutation.go), [routing engine](../reference/go/internal/objecter/client.go) | Preserve request identity, retained payloads, ambiguity and watermark invariants | Owned request records that outlive dropped Rust futures |
| [OSD operations](../reference/go/internal/osd/messages.go) and adjacent metadata/lock/watch modules | Reuse opcode/result semantics, compound payloads and tests | Typed consuming builders and byte-preserving Rust identities |
| [Integration infrastructure](../reference/go/integration/README.md), P03-P11 runners/probes/native drivers | Keep cluster topology, safety checks, native environment and scenario meaning | Add a Rust probe adapter per phase; retain Go path and reports |
| [P12 qualification contract](../reference/go/internal/p12qualcontract/contract.go), [qualifier](../reference/go/tools/p12-qualify/main.go) | Copy the evidence discipline and fixed-gate model | Rust-specific command, target, dependency and artifact contract; not string replacement |
| [P12 endurance runner](../reference/go/integration/p12/reproduce.sh), [report schema](../reference/go/integration/p12/report.schema.json) | Reuse scenarios, renewal/churn requirements and benchmark shape | Rust candidate/report namespace and worker/process invocation |
| [Review contract](../reference/go/docs/p12/human-review.md), [signature verifier](../reference/go/tools/p12-verify/review_signature_test.go) | Reuse role separation, exact-candidate binding and negative tests | Fresh Rust approvals and artifact hashes; never reuse Go signatures |
...
Model the race points between admission, dispatch, first transmitted bytes, ACK, OSD reply, cancellation and completion. Port the tests around [Go mutation admission and flush](../reference/go/internal/objecter/mutation.go), and add Rust-only tests that repeatedly poll/drop futures at each boundary. Validate the dispatch/replay rules against [messenger transport](../reference/go/internal/msgr/transport.go) and [OSD sessions](../reference/go/internal/objecter/osd_session.go), not the wrapper alone.
...
Use a separate immutable Rust candidate, report schemas, source/artifact hashes and accountable approvals for security, distributed systems, license/notices and release ownership, following [the existing review model](../reference/go/docs/p12/human-review.md). Review signatures bind the exact Rust candidate; private signing keys remain external to tooling and the LLM. Candidate production must not depend on a signature over itself, and reports must avoid self-referential hash cycles.
...
The project currently declares LGPL-2.1-only in [its license](../LICENSE) and [dependency audit](../reference/go/docs/p12/dependencies.md). The Rust port is expected to adapt Go code and upstream-derived implementations, not constitute a clean-room rewrite. Preserve applicable notices and per-file provenance; do not relabel it MIT/Apache merely because that is common for Rust crates.
