...
fn main() {
    ...
    let result = match command.to_str() {
        Some("default-request") => {
            ...
            rados_r06_tools::default_request_json(Path::new(&root)).map(|value| print!("{value}"))
        }
        Some("verify") => {
            ...
            rados_r06_tools::verify_report_file(Path::new(&root), Path::new(&report))
                .map(|()| println!("R06 bridge report verification passed"))
        }
        Some("rust-source-digest" | "go-source-digest") => {
            ...
            if command == "rust-source-digest" {
                rados_r06_tools::rust_source_digest(Path::new(&root))
            } else {
                rados_r06_tools::go_source_digest(Path::new(&root))
            }
        ...
        Some("path-digest") => {
            ...
            rados_r06_tools::path_digest(Path::new(&root), Path::new(&path))
                .map(|value| println!("{value}"))
        }
        _ => usage(&program),
    };
    if let Err(error) = result {
        eprintln!("rados-r06-verify: {error}");
        std::process::exit(1);
    }
}
