...
fn main() {
    ...
    let result = match remainder.as_slice() {
        [] => rados_r11_tools::verify_report_file(Path::new(&root), Path::new(&report)),
        [
            rust_binary,
            go_root,
            native_driver,
            native_binary,
            server_binary,
        ] => rados_r11_tools::verify_report_artifacts(
            Path::new(&root),
            Path::new(&report),
    ...
    if let Err(error) = result {
        eprintln!("rados-r11-verify: {error}");
        std::process::exit(1);
    }
    println!("R11 report verification passed");
}
