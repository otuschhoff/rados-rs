...
fn main() {
    ...
    if first.as_deref() == Some(std::ffi::OsStr::new("source-digest")) {
        ...
        match rados_r02_tools::source_digest(Path::new(&root)) {
    ...
    if first.as_deref() == Some(std::ffi::OsStr::new("path-digest")) {
        ...
        match rados_r02_tools::path_digest(Path::new(&root), Path::new(&path)) {
            Ok(digest) => println!("{digest}"),
            Err(error) => fail(&error),
        }
        return;
    ...
    if let Err(error) = rados_r02_tools::verify_report_file(Path::new(&root), Path::new(&report)) {
        fail(&error);
    }
    println!("R02 bridge report verification passed");
}
...
fn fail(error: &str) -> ! {
    eprintln!("rados-r02-verify: {error}");
    std::process::exit(1);
}
