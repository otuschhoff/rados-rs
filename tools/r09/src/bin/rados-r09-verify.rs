...
fn main() {
    ...
    let result = match remainder.as_slice() {
        [] => rados_r09_tools::verify_report_file(Path::new(&root), Path::new(&report)),
        [rust_binary, go_root, go_binary, native_binary] => {
            rados_r09_tools::verify_report_artifacts(
                Path::new(&root),
                Path::new(&report),
    ...
    if let Err(error) = result {
        eprintln!("rados-r09-verify: {error}");
        std::process::exit(1);
    }
    println!("R09 report verification passed");
}
