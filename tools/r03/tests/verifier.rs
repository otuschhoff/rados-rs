...
use rados_r03_tools::{
    GO_REVISION, GO_TREE, MAX_REPORT_BYTES, clean_go_command_for_tests, default_request_json,
    file_digest, hex_digest, path_digest, run_rust_probe, rust_source_digest,
    verify_report_bytes_for_tests, verify_report_bytes_with_timeout_for_tests,
};
...
fn valid_report() -> (serde_json::Value, PathBuf, PathBuf, PathBuf, PathBuf) {
    ...
    let fixture = |case_id: &str, path: Option<&str>, sha256: &str| {
        let manifest = path.map(|value| format!("{value}.json"));
        serde_json::json!({"case_id":case_id,"path":path,"sha256":sha256,"manifest_path":manifest,"manifest_sha256":manifest.as_ref().map(|value| file_digest(&root.join(value)).expect("manifest digest"))})
    };
    let session_sha = hex_digest(b"start-ready\nstop\n");
    ...
    let report = serde_json::json!({
        "schema_version":1,"suite_id":"r03/messenger-transcript-v1","status":"passed","generated_at":"2026-09-17T00:00:00Z",
        "bounds":{"max_record_bytes":16384,"max_records":6,"max_input_bytes":4096,"max_output_bytes":8192},
        "fixtures":[
            fixture("banner",Some("testdata/p02/banner-rev1.bin"),"6819c56d3d3d3ccaa0545a80d55aaa8088d88e9f849113cdafa58feb298cfcca"),
            fixture("crc-frame",Some("testdata/p02/upstream/upstream-crc-four-segment.bin"),"0fa63a785151214c426f128141faf3969eb0be008b9229505ab7146401dd8c42"),
            fixture("secure-frame",Some("testdata/p02/upstream/upstream-secure-one-segment.bin"),"592152e22495cecb210cf5a7bfefac49f6cdd8bbbb14e223db3538bef0b3926d"),
            fixture("control-ack",Some("testdata/p02/upstream/upstream-ack-control.bin"),"2496a22ad0abda5947019dcdbd5dfe482eab124a649d302cba119df923a6ce70"),
            fixture("message-frame",Some("testdata/p02/upstream/upstream-message-frame.bin"),"00a22987b019e085cf811b10852000116e3c63adc33bf86729485a2ecb81ff26"),
            fixture("session-transition",None,&session_sha)],
        ...
        "go":{"implementation_id":"go","source_revision":GO_REVISION,"source_tree":GO_TREE,"source_files_sha256":"0".repeat(64),"lockfile_sha256":"0".repeat(64),"compiler":go_compiler,"target":TEST_GO_TARGET,"driver_sha256":path_digest(&root,Path::new("tools/r03/go-probe")).expect("adapter"),"command":["go","run","./tools/rados-rs-r03-probe","--fixture-root","./tools/rados-rs-r03-probe/fixtures"],"exit_code":0,"stdout_sha256":records_digest(&output,"go"),"records":go_records},
        "artifacts":{"adapter_path":"tools/r03/go-probe","adapter_sha256":path_digest(&root,Path::new("tools/r03/go-probe")).expect("adapter"),"schema_path":"integration/r03/report.schema.json","schema_sha256":file_digest(&root.join("integration/r03/report.schema.json")).expect("schema")},
        "controller":{"path":"integration/r03/reproduce.sh","sha256":file_digest(&root.join("integration/r03/reproduce.sh")).expect("controller"),"command":["integration/r03/reproduce.sh","--go-root","/unused/go","--rust-probe",probe.to_string_lossy(),"--verifier",verifier.to_string_lossy(),"--report",report_path.to_string_lossy()]}
    });
...
fn verifier_ignores_poisoned_go_environment() {
    const CHILD_MARKER: &str = "RADOS_R03_POISONED_GO_ENV_CHILD";
