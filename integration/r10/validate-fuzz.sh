...
rm -rf "$artifacts"
mkdir -p "$artifacts"
for target in r10_class r10_lock r10_watch; do
...
[ "$source_sha256" = "$(source_digest)" ] || { printf '%s\n' 'R10 fuzz: source changed during campaign' >&2; exit 1; }
...
jq -s --arg started_at "$started_at" --arg finished_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg source_sha256 "$source_sha256" --arg rustc "$($rustc --version)" \
  '{schema_version:1,suite_id:"r10/fuzz-task-validation-v1",status:"passed",started_at:$started_at,finished_at:$finished_at,source_sha256:$source_sha256,rustc:$rustc,cargo_fuzz:"cargo-fuzz 0.13.2",campaigns:.}' "$temporary/results.jsonl" >"$report"
cargo run --quiet -p rados-r10-tools --bin rados-r10-verify-fuzz -- "$root" "$report"
printf '%s\n' "R10 fuzz validation passed: $report"
