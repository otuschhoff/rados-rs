...
source_digest() { git -C "$root" ls-files -co --exclude-standard -- 'src/**' 'tools/r12/**' 'integration/r12/**' 'fuzz/fuzz_targets/r12_*' Cargo.toml Cargo.lock build.rs rust-toolchain.toml | LC_ALL=C sort | while IFS= read -r file; do printf '%s  %s\n' "$(sha256_file "$root/$file")" "$file"; done | shasum -a 256 | awk '{print $1}'; }
...
rm -rf "$artifacts"
mkdir -p "$artifacts"
for target in r12_command r12_stats r12_inconsistent; do
...
[ "$source_sha256" = "$(source_digest)" ] || { printf '%s\n' 'R12 fuzz: source changed during campaign' >&2; exit 1; }
...
jq -s --arg started_at "$started_at" --arg finished_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg source_sha256 "$source_sha256" --arg rustc "$($rustc --version)" \
  '{schema_version:1,suite_id:"r12/fuzz-task-validation-v1",status:"passed",started_at:$started_at,finished_at:$finished_at,source_sha256:$source_sha256,rustc:$rustc,cargo_fuzz:"cargo-fuzz 0.13.2",campaigns:.}' "$temporary/results.jsonl" >"$report"
cargo run --quiet -p rados-r12-tools --bin rados-r12-verify-fuzz -- "$root" "$report"
printf '%s\n' "R12 fuzz validation passed: $report"
