...
rm -rf "$artifacts"
mkdir -p "$artifacts"
for target in r09_metadata r09_compound r09_enumeration; do
...
[ "$source_sha256" = "$(source_digest)" ] || { printf '%s\n' 'R09 fuzz: source changed during campaign' >&2; exit 1; }
...
jq -s --arg started_at "$started_at" --arg finished_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg source_sha256 "$source_sha256" --arg rustc "$($rustc --version)" \
  '{schema_version:1,suite_id:"r09/fuzz-task-validation-v1",status:"passed",started_at:$started_at,finished_at:$finished_at,source_sha256:$source_sha256,rustc:$rustc,cargo_fuzz:"cargo-fuzz 0.13.2",campaigns:.}' "$temporary/results.jsonl" >"$report"
cargo run --quiet -p rados-r09-tools --bin rados-r09-verify-fuzz -- "$root" "$report"
printf '%s\n' "R09 fuzz validation passed: $report"
